...
CREATE TABLE public.sivel2_gen_victima (
    ...
    CONSTRAINT victima_orientacionsexual_check CHECK (((orientacionsexual = 'L'::bpchar) OR (orientacionsexual = 'G'::bpchar) OR (orientacionsexual = 'B'::bpchar) OR (orientacionsexual = 'T'::bpchar) OR (orientacionsexual = 'H'::bpchar) OR (orientacionsexual = 'S'::bpchar) OR (orientacionsexual = 'I'::bpchar)))
);
...
CREATE VIEW public.cmunex AS
 SELECT ( SELECT sip_pais.nombre
   ...
   FROM public.sivel2_sjr_desplazamiento desplazamiento,
    ...
    public.sivel2_gen_victima victima,
    public.sivel2_sjr_casosjr casosjr
  WHERE ((casosjr.id_caso = desplazamiento.id_caso) AND (desplazamiento.id_caso = victima.id_caso) AND (casosjr.oficina_id = 7) AND (desplazamiento.id_expulsion_porborrar = ubicacion.id));
...
CREATE VIEW public.cmunrec AS
 SELECT ( SELECT sip_pais.nombre
   ...
   FROM public.sivel2_sjr_desplazamiento desplazamiento,
    ...
    public.sivel2_gen_victima victima,
    public.sivel2_sjr_casosjr casosjr
  WHERE ((casosjr.id_caso = desplazamiento.id_caso) AND (desplazamiento.id_caso = victima.id_caso) AND (casosjr.oficina_id = 7) AND (desplazamiento.id_llegada_porborrar = ubicacion.id));
...
CREATE MATERIALIZED VIEW public.consgifmm AS
 SELECT detallefinanciero.id,
    ...
    ARRAY( SELECT detallefinanciero_persona.persona_id
           FROM public.detallefinanciero_persona
          WHERE (detallefinanciero_persona.detallefinanciero_id = detallefinanciero.id)) AS persona_ids,
...
CREATE MATERIALIZED VIEW public.sivel2_gen_consexpcaso AS
 SELECT conscaso.caso_id,
  ...
  WHERE (conscaso.caso_id IN ( SELECT sivel2_gen_conscaso.caso_id
          ...
          WHERE ((sivel2_sjr_casosjr.contacto_id = sip_persona.id) AND ((sip_persona.apellidos)::text ~~* '%bbb%'::text) AND (sivel2_gen_conscaso.fecharec >= '2021-01-01'::date) AND ((sivel2_sjr_casosjr.contacto_id = sip_persona.id) AND ((sip_persona.nombres)::text ~~* '%aaa%'::text)))
