...
CREATE VIEW cben1 AS
 SELECT caso.id AS id_caso,
   ...
   FROM caso,
    casosjr,
    victima
  WHERE ((((((caso.id <> (-1)) AND (caso.id = casosjr.id_caso)) AND (caso.id = victima.id_caso)) AND (casosjr.fecharec >= '1999-01-01'::date)) AND (casosjr.fecharec <= '2014-12-31'::date)) AND (casosjr.id_regionsjr = 4));
...
CREATE VIEW cben2 AS
 SELECT cben1.id_caso,
    ...
    max(desplazamiento.fechaexpulsion) AS max
   FROM (((((cben1
     LEFT JOIN desplazamiento ON ((cben1.id_caso = desplazamiento.id_caso)))
     LEFT JOIN ubicacion ON ((desplazamiento.id_expulsion = ubicacion.id)))
     LEFT JOIN departamento ON ((ubicacion.id_departamento = departamento.id)))
     LEFT JOIN municipio ON (((ubicacion.id_municipio = municipio.id) AND (ubicacion.id_departamento = municipio.id_departamento))))
     LEFT JOIN clase ON ((((ubicacion.id_clase = clase.id) AND (ubicacion.id_municipio = clase.id_municipio)) AND (ubicacion.id_departamento = clase.id_departamento))))
  GROUP BY cben1.id_caso, cben1.id_persona, cben1.contacto, cben1.beneficiario, cben1.total, ubicacion.id_departamento, departamento.nombre, ubicacion.id_municipio, municipio.nombre, ubicacion.id_clase, clase.nombre;
...
CREATE VIEW cons AS
 SELECT DISTINCT victima.id_persona,
   ...
   FROM victima,
    ...
    caso,
    categoria
  WHERE ((((((((((((categoria.id = acto.id_categoria) AND (caso.id <> (-1))) AND (caso.fecha >= '1990-01-01'::date)) AND (caso.fecha <= '2024-11-30'::date)) AND (caso.fecha >= '2013-01-01'::date)) AND (caso.fecha <= '2014-05-05'::date)) AND (victima.id_caso = caso.id)) AND (victima.id_persona = persona.id)) AND (persona.id = acto.id_persona)) AND (acto.id_caso = caso.id)) AND (acto.id_categoria = categoria.id)) AND ((categoria.id_tviolencia)::text = 'S'::text));
...
CREATE VIEW y AS
 SELECT caso.id,
    min(caso_usuario.fechainicio) AS min
   FROM caso_usuario,
    caso
